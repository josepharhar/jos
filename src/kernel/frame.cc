...
// ...|s___|____|____|e...
static void InitializeFrameRegion(uint64_t start_address,
...
void FrameInit(struct TagsInfo tags_info) {
  ...
  uint64_t avl_one_end = AlignAddressDown(tags_info.region_one_address +
                                          tags_info.region_one_size);
  ...
  uint64_t avl_two_end = AlignAddressDown(tags_info.region_two_address +
                                          tags_info.region_two_size);
  uint64_t num_regions = (avl_two_end - avl_two_start) / REGION_MAX_SIZE_BYTES;
  // throw away extra space after the last region so we don't have to keep track
  // of how big the regions are
  // uint64_t last_region_size = (avl_two_end - avl_two_start) %
  // REGION_MAX_SIZE_BYTES; // if zero, no extra region
  ...
  uint8_t* last_region_pointer =
      start_pointer + REGION_MAX_SIZE_BYTES * (num_regions - 1);
